...
CODE_DIR="$(agmsg_skill_dir)"
SKILL_DIR="$(agmsg_runtime_dir)"
SKILL_NAME="$(basename "$CODE_DIR")"
...
apply_settings_copilot() {
  ...
  if [ "$mode" = "turn" ]; then
    ...
    local cmd="'$CODE_DIR/scripts/check-inbox.sh' '$type' '$project'"
...
- Command: '$CODE_DIR/scripts/check-inbox.sh' '$type' '$project'
EOF
      ;;
    monitor)
...
apply_settings() {
  ...
  case "$mode" in
    monitor)
      local ss="'$CODE_DIR/scripts/session-start.sh' '$type' '$project'"
      local se="'$CODE_DIR/scripts/session-end.sh'   '$type' '$project'"
    ...
    both)
      local ss="'$CODE_DIR/scripts/session-start.sh' '$type' '$project'"
      local se="'$CODE_DIR/scripts/session-end.sh'   '$type' '$project'"
      local st="'$CODE_DIR/scripts/check-inbox.sh'   '$type' '$project'"
      add_event_entry_file "$tmp_state" "SessionStart" "$ss" "$type"
      add_event_entry_file "$tmp_state" "SessionEnd"   "$se" "$type"
...
emit_monitor_directive() {
  local type="$1"
  local project="$2"
  local watch="$CODE_DIR/scripts/watch.sh"
