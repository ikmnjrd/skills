...
# `run/cc-instance.<cc_pid>` under the configured runtime, recording the last
  ...
  if [ -n "$dead_sid" ] \
    ...
    if [ -f "$orphan_pidfile" ]; then
      ...
      if [ -n "$orphan_pid" ] && kill -0 "$orphan_pid" 2>/dev/null; then
        ...
        case "$cmd" in
          *"$CODE_DIR/scripts/watch.sh"*) kill "$orphan_pid" 2>/dev/null || true ;;
...
WATCH="$CODE_DIR/scripts/watch.sh"
