...
ユーザーの指定に合わせて、Claude へ指示する対象を決めます。差分の取得と確認は Claude に行わせ、Codex 側で `git diff` やファイル本文を先に集めて渡しません。

ユーザーが対象を指定していない場合は、未コミットの作業ツリー差分をレビュー対象にします。

- 対象指定なし: 未コミットの作業ツリー差分
- staged diff: staged 差分
- 直近コミット: `HEAD^..HEAD`
- 任意のコミット: `<commit>^..<commit>`
- コミット範囲: `<base>..<head>`
- 設計や計画: ユーザーの説明、関連ファイル、既知の制約

差分が大きい可能性がある場合も、まず Claude に `git status`、`git diff`、`git log` で確認させます。Codex が差分本文を要約してから渡すのは、Claude の実行が失敗した後にユーザーが明示的に求めた場合だけにします。

### 2. Claude に渡すプロンプトを作る
...
対象: <未コミット差分 / staged diff / HEAD^..HEAD / commit range / plan>
現在の状況: <summary and tests>

確認方法:
- 差分、ファイル一覧、関連ファイル本文は、許可された Git / Read / Grep / Glob ツールであなたが確認してください。
- 対象指定がない場合は、未コミットの作業ツリー差分をレビューしてください。
- ファイルは編集しないでください。
```

### 3. `claude -p` をサンドボックス外で実行する
...
対象指定がない場合のプロンプトでは、対象を「未コミットの作業ツリー差分」と明記し、Claude に `git status` と `git diff` で確認させます。

プロンプトが長い場合は、標準入力を使います。
...
- `claude: command not found`、`unknown option` など CLI 自体の失敗: コマンドの要点と失敗理由を報告して止めます。
- `loggedIn: false`、`Not logged in`、`Please run /login` など認証失敗: 通常の Claude Code 環境でログインが必要だとユーザーに報告して止めます。
- 権限、ネットワーク、予算、その他 CLI エラー: 失敗理由を報告して止めます。

失敗時に agmsg や `spawn` へフォールバックしません。
